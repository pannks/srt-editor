...
animation. Alongside fade and pop are three word-timed modes — **karaoke**
(fill each word as it is spoken), **highlight word** and **one word at a time** —
which split even spaceless scripts like Thai into real words with the browser's
dictionary segmenter. One-click **templates** apply a whole look, and the live
preview matches the burn pixel for pixel with ffmpeg's libass rendering the final
video.
...
11. **Caption Studio** — the toolbar's *Create captions* tab. Start from a **template** (a one-click gallery of looks — Clean, Karaoke, Word punch, Highlight, Neon, Tall caps and two Thai presets), then add one or more caption layers over the video; each layer is fully styled on its own — language (source or any translation), font (system or a Google family fetched for both preview and export), size as a percent of frame height, bold, text colour, highlight (accent) colour, outline width and colour, drop shadow, an optional background box, anchor position dragged live over the picture (or snapped with *Centre across* / *Centre on screen*), horizontal/vertical alignment, wrap width, and an animation. The animations are fade, pop, and three word-timed modes — **karaoke** (each word fills from the text colour to the highlight colour as it is spoken), **highlight word** (the current word takes the accent) and **one word** (a single word pops in at a time); all three split the line into real words, using the browser's dictionary segmenter so Thai and other spaceless scripts get true per-word timing. The preview sizes and wraps exactly as the burn will — libass renders text so the font's win metric fills the size, which the export corrects per font so Thai/CJK layers are not shrunk. **Export video** burns every layer into a new MP4 with ffmpeg + libass; progress shows in the toolbar and the Process log.
