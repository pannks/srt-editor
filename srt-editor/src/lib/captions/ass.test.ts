...
describe("buildAss", () => {
  it("emits a playres header matching the video", () => {
    const out = buildAss([block(0, 1, "hi")], DEFAULT_CAPTION_STYLE, dims);
  ...
  it("uses the chosen translation line", () => {
    const out = buildAss(
      ...
      { ...DEFAULT_CAPTION_STYLE, language: "th" },
      dims,
    );
  ...
  it("adds fade tags when asked", () => {
    const out = buildAss(
      [block(0, 1, "hi")],
      { ...DEFAULT_CAPTION_STYLE, animation: "fade" },
      dims,
    );
  ...
  it("switches to an opaque box with the requested opacity", () => {
    const out = buildAss(
      [block(0, 1, "hi")],
      { ...DEFAULT_CAPTION_STYLE, bgEnabled: true, bgOpacity: 0.5 },
      dims,
    );
  ...
  it("emits karaoke word timings", () => {
    const out = buildAss(
      [block(0, 2, "two words")],
      { ...DEFAULT_CAPTION_STYLE, animation: "karaoke" },
      dims,
    );
