...
| Import/export WebVTT | `src/lib/vtt/` mirrors the SRT parser/generator; opens `.vtt` (dialog + drag-drop), and the Export menu has an SRT/VTT toggle covering every language variant |

## Next up (recommended order)

1. **Relink missing media after importing `.srtproj`** — the import path
...
2. **Subtitle quality checks** — flag cues over N chars/line, reading speed
   ...
   export targets.
3. **Translation-stale indicator** — hash the source text into each
   translation entry when it is written; show a badge when they diverge, and
   let "Translate" re-run only stale blocks.
4. **Autosave version history** — periodic snapshots of the open project into
